...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("providers", parameters);
            }
        };

        context.registerService?.("providers", service);
        return service;
    }
    ...
    const commands = [{
        name: "providers",
        category: "providers",
        description: "List and search all configured providers.",
        usage: "providers [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("providers");
