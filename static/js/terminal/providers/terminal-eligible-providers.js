...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("providers/eligible", parameters);
            }
        };

        context.registerService?.("eligible-providers", service);
        return service;
    }
    ...
    const commands = [{
        ...
        category: "providers",
        description: "List providers eligible for ingestion.",
        usage: "eligible-providers [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("eligible-providers");
