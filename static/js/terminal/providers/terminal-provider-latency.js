...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("providers/latency", parameters);
            }
        };

        context.registerService?.("provider-latency", service);
        return service;
    }
    ...
    const commands = [{
        ...
        category: "providers",
        description: "Inspect provider response and ingestion latency.",
        usage: "provider-latency [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("provider-latency");
