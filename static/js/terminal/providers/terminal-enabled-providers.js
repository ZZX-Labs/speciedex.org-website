...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("providers/enabled", parameters);
            }
        };

        context.registerService?.("enabled-providers", service);
        return service;
    }
    ...
    const commands = [{
        ...
        category: "providers",
        description: "List providers enabled in the current build.",
        usage: "enabled-providers [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("enabled-providers");
