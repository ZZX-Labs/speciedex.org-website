...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("providers/statistics", parameters);
            }
        };

        context.registerService?.("provider-statistics", service);
        return service;
    }
    ...
    const commands = [{
        ...
        category: "providers",
        description: "Display provider-level statistics.",
        usage: "provider-statistics [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("provider-statistics");
