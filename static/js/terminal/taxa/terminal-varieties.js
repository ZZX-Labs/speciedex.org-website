...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("taxa/varieties", parameters);
            }
        };

        context.registerService?.("varieties", service);
        return service;
    }
    ...
    const commands = [{
        name: "varieties",
        category: "taxonomy",
        description: "Search taxonomic varieties.",
        usage: "varieties [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("varieties");
