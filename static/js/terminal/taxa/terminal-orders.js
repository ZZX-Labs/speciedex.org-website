...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("taxa/orders", parameters);
            }
        };

        context.registerService?.("orders", service);
        return service;
    }
    ...
    const commands = [{
        name: "orders",
        category: "taxonomy",
        description: "Search taxonomic orders.",
        usage: "orders [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("orders");
