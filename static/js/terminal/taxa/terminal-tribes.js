...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("taxa/tribes", parameters);
            }
        };

        context.registerService?.("tribes", service);
        return service;
    }
    ...
    const commands = [{
        name: "tribes",
        category: "taxonomy",
        description: "Search taxonomic tribes.",
        usage: "tribes [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("tribes");
