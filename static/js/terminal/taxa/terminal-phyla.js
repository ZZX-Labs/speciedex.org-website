...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("taxa/phyla", parameters);
            }
        };

        context.registerService?.("phyla", service);
        return service;
    }
    ...
    const commands = [{
        name: "phyla",
        category: "taxonomy",
        description: "Search taxonomic phyla.",
        usage: "phyla [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("phyla");
