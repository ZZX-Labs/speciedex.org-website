...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("taxa/ranks", parameters);
            }
        };

        context.registerService?.("ranks", service);
        return service;
    }
    ...
    const commands = [{
        name: "ranks",
        category: "taxonomy",
        description: "List supported taxonomic ranks.",
        usage: "ranks [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("ranks");
    ...
    window.SpeciedexTerminalRanks = api;
