...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("taxa/kingdoms", parameters);
            }
        };

        context.registerService?.("kingdoms", service);
        return service;
    }
    ...
    const commands = [{
        ...
        usage: "kingdoms [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("kingdoms");
