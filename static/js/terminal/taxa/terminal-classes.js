...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("taxa/classes", parameters);
            }
        };

        context.registerService?.("classes", service);
        return service;
    }
    ...
    const commands = [{
        name: "classes",
        category: "taxonomy",
        description: "Search taxonomic classes.",
        usage: "classes [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("classes");
