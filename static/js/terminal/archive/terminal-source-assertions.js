...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("archive/assertions", parameters);
            }
        };

        context.registerService?.("source-assertions", service);
        return service;
    }
    ...
    const commands = [{
        ...
        category: "archive",
        description: "Inspect source assertion records.",
        usage: "source-assertions [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("source-assertions");
    ...
    window.SpeciedexTerminalSourceAssertions = api;
