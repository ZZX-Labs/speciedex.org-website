...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("archive/manifests", parameters);
            }
        };

        context.registerService?.("manifests", service);
        return service;
    }
    ...
    const commands = [{
        name: "manifests",
        category: "archive",
        description: "Inspect archive manifests.",
        usage: "manifests [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("manifests");
    ...
    window.SpeciedexTerminalManifests = api;
