...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("archive/conflicts", parameters);
            }
        };

        context.registerService?.("unresolved-conflicts", service);
        return service;
    }
    ...
    const commands = [{
        ...
        category: "archive",
        description: "Inspect unresolved provider conflicts.",
        usage: "unresolved-conflicts [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("unresolved-conflicts");
    ...
    window.SpeciedexTerminalUnresolvedConflicts = api;
