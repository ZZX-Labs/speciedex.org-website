...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("archive/volumes", parameters);
            }
        };

        context.registerService?.("volumes", service);
        return service;
    }
    ...
    const commands = [{
        name: "volumes",
        category: "archive",
        description: "List archive volumes and metadata.",
        usage: "volumes [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("volumes");
    ...
    window.SpeciedexTerminalVolumes = api;
