...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("archive/checksums", parameters);
            }
        };

        context.registerService?.("checksums", service);
        return service;
    }
    ...
    const commands = [{
        name: "checksums",
        category: "archive",
        description: "Inspect archive checksums and integrity records.",
        usage: "checksums [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("checksums");
    ...
    window.SpeciedexTerminalChecksums = api;
