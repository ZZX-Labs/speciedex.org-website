...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("archive/synonyms", parameters);
            }
        };

        context.registerService?.("synonyms", service);
        return service;
    }
    ...
    const commands = [{
        name: "synonyms",
        category: "archive",
        description: "Search archived taxonomic synonyms.",
        usage: "synonyms [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("synonyms");
    ...
    window.SpeciedexTerminalSynonyms = api;
