...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("archive/records", parameters);
            }
        };

        context.registerService?.("records-archived", service);
        return service;
    }
    ...
    const commands = [{
        ...
        category: "archive",
        description: "Display canonical archived record totals.",
        usage: "records-archived [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("records-archived");
    ...
    window.SpeciedexTerminalRecordsArchived = api;
