...
(function (window, document) {
    ...
    function initialize(context) {
        const service = {
            async list(parameters = {}) {
                ...
                return context.api.get("archive/last-updated", parameters);
            }
        };

        context.registerService?.("last-updated", service);
        return service;
    }
    ...
    const commands = [{
        name: "last-updated",
        category: "archive",
        description: "Display archive and provider update timestamps.",
        usage: "last-updated [query] [limit]",
        handler: async ({ args, context, writeJSON }) => {
            const query = args[0] || "";
            const limit = Number(args[1]) || 50;
            const service = context.services?.get("last-updated");
    ...
    window.SpeciedexTerminalLastUpdated = api;
