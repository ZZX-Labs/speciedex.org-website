...
(function (window, document) {
    ...
    function render(data = [], options = {}) {
        ...
        container.className = "terminal-visualization terminal-visualization-forcegraph";
        container.dataset.visualization = "forcegraph";
        container.setAttribute("role", "img");
        container.setAttribute(
            "aria-label",
            options.label || "ForceGraph visualization"
        );
        ...
        context2d.fillText("ForceGraph", 24, 32);
    ...
    function initialize(context) {
        ...
        context.registerVisualization?.("forcegraph", visualization);
        context.registerRenderer?.("forcegraph", visualization);
    ...
    const commands = [{
        ...
        description: "Render the ForceGraph visualization.",
        usage: "forcegraph [collection]",
        handler: ({ args, context }) => {
            const collection = args[0] || "records";
            const data = context.library?.get(collection) || [];
            return render(data, { label: "ForceGraph for " + collection });
        }
    }];
