...
(function (window, document) {
    ...
    function render(data = [], options = {}) {
        ...
        container.className = "terminal-visualization terminal-visualization-constellation";
        container.dataset.visualization = "constellation";
        container.setAttribute("role", "img");
        container.setAttribute(
            "aria-label",
            options.label || "Constellation visualization"
        );
        ...
        context2d.fillText("Constellation", 24, 32);
    ...
    function initialize(context) {
        ...
        context.registerVisualization?.("constellation", visualization);
        context.registerRenderer?.("constellation", visualization);
    ...
    const commands = [{
        ...
        description: "Render the Constellation visualization.",
        usage: "constellation [collection]",
        handler: ({ args, context }) => {
            const collection = args[0] || "records";
            const data = context.library?.get(collection) || [];
            return render(data, { label: "Constellation for " + collection });
        }
    }];
    ...
    window.SpeciedexTerminalConstellation = api;
