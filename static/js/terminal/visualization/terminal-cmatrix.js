...
(function (window, document) {
    ...
    function render(data = [], options = {}) {
        ...
        container.setAttribute(
            "aria-label",
            options.label || "CMatrix visualization"
        );
        ...
        context2d.fillText("CMatrix", 24, 32);
    ...
    const commands = [{
        ...
        description: "Render the CMatrix visualization.",
        usage: "cmatrix [collection]",
        handler: ({ args, context }) => {
            const collection = args[0] || "records";
            const data = context.library?.get(collection) || [];
            return render(data, { label: "CMatrix for " + collection });
        }
    }];
