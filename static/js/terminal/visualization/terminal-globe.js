...
(function (window, document) {
    ...
    function render(data = [], options = {}) {
        ...
        container.className = "terminal-visualization terminal-visualization-globe";
        container.dataset.visualization = "globe";
        container.setAttribute("role", "img");
        container.setAttribute(
            "aria-label",
            options.label || "Globe visualization"
        );
        ...
        context2d.fillText("Globe", 24, 32);
    ...
    function initialize(context) {
        ...
        context.registerVisualization?.("globe", visualization);
        context.registerRenderer?.("globe", visualization);
    ...
    const commands = [{
        ...
        description: "Render the Globe visualization.",
        usage: "globe [collection]",
        handler: ({ args, context }) => {
            const collection = args[0] || "records";
            const data = context.library?.get(collection) || [];
            return render(data, { label: "Globe for " + collection });
        }
    }];
    ...
    window.SpeciedexTerminalGlobe = api;
