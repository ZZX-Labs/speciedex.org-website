...
(function (window, document) {
    ...
    function render(data = [], options = {}) {
        ...
        container.className = "terminal-visualization terminal-visualization-sankey";
        container.dataset.visualization = "sankey";
        container.setAttribute("role", "img");
        container.setAttribute(
            "aria-label",
            options.label || "Sankey visualization"
        );
        ...
        context2d.fillText("Sankey", 24, 32);
    ...
    function initialize(context) {
        ...
        context.registerVisualization?.("sankey", visualization);
        context.registerRenderer?.("sankey", visualization);
    ...
    const commands = [{
        ...
        description: "Render the Sankey visualization.",
        usage: "sankey [collection]",
        handler: ({ args, context }) => {
            const collection = args[0] || "records";
            const data = context.library?.get(collection) || [];
            return render(data, { label: "Sankey for " + collection });
        }
    }];
    ...
    window.SpeciedexTerminalSankey = api;
