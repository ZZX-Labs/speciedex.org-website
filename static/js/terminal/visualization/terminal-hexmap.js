...
(function (window, document) {
    ...
    function render(data = [], options = {}) {
        ...
        container.className = "terminal-visualization terminal-visualization-hexmap";
        container.dataset.visualization = "hexmap";
        container.setAttribute("role", "img");
        container.setAttribute(
            "aria-label",
            options.label || "HexMap visualization"
        );
        ...
        context2d.fillText("HexMap", 24, 32);
    ...
    function initialize(context) {
        ...
        context.registerVisualization?.("hexmap", visualization);
        context.registerRenderer?.("hexmap", visualization);
    ...
    const commands = [{
        ...
        description: "Render the HexMap visualization.",
        usage: "hexmap [collection]",
        handler: ({ args, context }) => {
            const collection = args[0] || "records";
            const data = context.library?.get(collection) || [];
            return render(data, { label: "HexMap for " + collection });
        }
    }];
    ...
    window.SpeciedexTerminalHexMap = api;
