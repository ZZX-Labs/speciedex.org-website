...
(function (window, document) {
    ...
    function render(data = [], options = {}) {
        ...
        container.className = "terminal-visualization terminal-visualization-radial";
        container.dataset.visualization = "radial";
        container.setAttribute("role", "img");
        container.setAttribute(
            "aria-label",
            options.label || "Radial visualization"
        );
        ...
        context2d.fillText("Radial", 24, 32);
    ...
    function initialize(context) {
        ...
        context.registerVisualization?.("radial", visualization);
        context.registerRenderer?.("radial", visualization);
    ...
    const commands = [{
        ...
        description: "Render the Radial visualization.",
        usage: "radial [collection]",
        handler: ({ args, context }) => {
            const collection = args[0] || "records";
            const data = context.library?.get(collection) || [];
            return render(data, { label: "Radial for " + collection });
        }
    }];
    ...
    window.SpeciedexTerminalRadial = api;
