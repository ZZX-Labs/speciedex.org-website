...
(function (window, document) {
    ...
    function render(data = [], options = {}) {
        ...
        container.className = "terminal-visualization terminal-visualization-network";
        container.dataset.visualization = "network";
        container.setAttribute("role", "img");
        container.setAttribute(
            "aria-label",
            options.label || "Network visualization"
        );
        ...
        context2d.fillText("Network", 24, 32);
    ...
    function initialize(context) {
        ...
        context.registerVisualization?.("network", visualization);
        context.registerRenderer?.("network", visualization);
    ...
    const commands = [{
        ...
        description: "Render the Network visualization.",
        usage: "network [collection]",
        handler: ({ args, context }) => {
            const collection = args[0] || "records";
            const data = context.library?.get(collection) || [];
            return render(data, { label: "Network for " + collection });
        }
    }];
    ...
    window.SpeciedexTerminalNetwork = api;
