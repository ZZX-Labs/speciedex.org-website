...
This scheduler is optimized for faster provider rotation while preserving
determinism, fairness, resumability, and failure isolation.

Responsibilities:

- rotate eligible providers fairly;
- preserve scheduler state between runs;
- honor explicit provider selections;
- honor --all-providers;
- enforce provider budgets;
- respect optional per-provider intervals;
- track success, failure, duration, records, and requests;
- apply bounded exponential failure backoff;
- prevent unhealthy providers from monopolizing scan time;
- prefer overdue providers without starving lower-priority providers;
- produce deterministic scheduler metadata.

The scheduler does not import, load, or execute providers. It only decides which
...
DEFAULT_PROVIDER_BUDGET = 8
DEFAULT_INTERVAL_MINUTES = 10
DEFAULT_FAILURE_BACKOFF_MINUTES = 10
DEFAULT_MAX_FAILURE_BACKOFF_MINUTES = 12 * 60
DEFAULT_MAX_CONSECUTIVE_FAILURES = 8

# Fairness and throughput controls.
DEFAULT_MAX_PROVIDER_SHARE = 1.0
DEFAULT_OVERDUE_WEIGHT = 25
DEFAULT_FAILURE_PENALTY = 35
DEFAULT_DURATION_PENALTY = 1
...
class ProviderScheduleState:
    ...
    def from_dict(
        ...
        return cls(
            provider=provider,
            last_selected=normalize_space(value.get("last_selected")),
            last_success=normalize_space(value.get("last_success")),
            last_failure=normalize_space(value.get("last_failure")),
            next_due=normalize_space(value.get("next_due")),
            consecutive_failures=max(
                0,
                _safe_int(value.get("consecutive_failures")),
            ),
            total_runs=max(
                0,
                _safe_int(value.get("total_runs")),
            ),
            total_successes=max(
                0,
                _safe_int(value.get("total_successes")),
            ),
            total_failures=max(
                0,
                _safe_int(value.get("total_failures")),
            ),
            last_error=normalize_space(value.get("last_error")),
            last_duration_seconds=_safe_float_or_none(
                value.get("last_duration_seconds")
            ),
            average_duration_seconds=_safe_float_or_none(
                value.get("average_duration_seconds")
            ),
            last_records=max(
                0,
                _safe_int(value.get("last_records")),
            ),
            total_records=max(
                0,
                _safe_int(value.get("total_records")),
            ),
            total_requests=max(
                0,
                _safe_int(value.get("total_requests")),
            ),
            empty_successes=max(
                0,
                _safe_int(value.get("empty_successes")),
            ),
        )
...
class SchedulerState:
    ...
    registered: int = 0
    eligible: int = 0
    last_selection: list[str] = field(default_factory=list)
    providers: dict[str, ProviderScheduleState] = field(
    ...
    def from_dict(cls, value: Any) -> "SchedulerState":
        ...
        providers: dict[str, ProviderScheduleState] = {}

        if isinstance(raw_providers, Mapping):
            for name, provider_state in raw_providers.items():
        ...
        raw_selection = value.get("last_selection")

        if not isinstance(raw_selection, list):
            raw_selection = []

        return cls(
            cursor=max(0, _safe_int(value.get("cursor"))),
...
class Scheduler:
    ...
    Provider definitions may optionally include:
        ...
        "interval_minutes": 10
        "empty_success_interval_minutes": 5
        "priority": 100
        "failure_backoff_minutes": 10
        "max_failure_backoff_minutes": 720
        "max_consecutive_failures": 8
        "schedule_enabled": true

    Priority influences ordering, but it does not permanently override fairness.
    Providers become increasingly favored as they become overdue.
    """
    ...
    def __init__(
        ...
        state_path: Path,
        *,
        default_budget: int = DEFAULT_PROVIDER_BUDGET,
        ...
        default_empty_success_interval_minutes: int = (
    ...
    def select(
        ...
        *,
        ...
        Explicit requests and --all-providers bypass interval and backoff
        scheduling. Normal scans rank due providers using a deterministic score
        composed of:
        ...
        - failure penalty;
        - average-duration penalty.

        This allows fast providers to rotate quickly while ensuring slow or
        ...
        cursor_before = (
            self.state.cursor % eligible_count
            if eligible_count
            else 0
        )
        ...
        if requested_names:
            ...
            for requested_name in sorted(
                requested_names - available_names
            ):
                skipped.append(
                    {
                        "provider": requested_name,
                        "reason": (
                            "requested provider is not eligible"
                        ),
                    }
                )

            cursor_after = self._cursor_after_selection(
                normalized_eligible,
                selected,
                cursor_before,
            )
            ...
            selected = list(normalized_eligible)
            ...
            due.sort(
                key=lambda definition: self._selection_sort_key(
        ...
        selected_at = format_timestamp(current)

        for definition in selected:
            name = normalize_key(definition.get("name"))
            state = self._provider_state(name)
            state.last_selected = selected_at

            # Selection reserves a short interval so two overlapping workers do
            # not immediately select the same provider before execution state
            # has been recorded.
        ...
        return SchedulerSelection(
            selected=[dict(definition) for definition in selected],
            deferred=deferred,
            skipped=skipped,
            cursor_before=cursor_before,
    ...
    def record_success(
        ...
        *,
        ...
        state.last_records = max(0, _safe_int(fetched))
        state.total_records += state.last_records
        state.total_requests += max(0, _safe_int(requests))

        if state.last_records == 0:
            state.empty_successes += 1
        else:
        ...
        if duration_seconds is not None:
            ...
            if (
                state.average_duration_seconds is None
                or previous_count <= 0
            ):
                state.average_duration_seconds = duration
            else:
                previous_average = (
    ...
    def provider_state(self, provider: str) -> dict[str, Any]:
        ...
        return self._provider_state(name).to_dict()

    def summary(self) -> dict[str, Any]:
        """Return compact scheduler health and throughput metadata."""

        provider_states = list(self.state.providers.values())
        ...
        return {
            ...
            "last_selection": list(self.state.last_selection),
            "providers_tracked": len(provider_states),
            "providers_in_backoff": sum(
                1
    ...
    def _selection_sort_key(
        ...
        *,
        ...
        Higher computed scores should run first, so the first tuple component
        is the negative score.
        """

        name = normalize_key(definition.get("name"))
        state = self._provider_state(name)
        priority = self._priority(definition)
        ...
        next_due = parse_timestamp(state.next_due)
        overdue_minutes = 0.0
        ...
        overdue_bonus = min(
            10_000.0,
            overdue_minutes * DEFAULT_OVERDUE_WEIGHT,
        )
    ...
    def _defer_reason(
        ...
        if not self._schedule_enabled(definition):
            return "provider scheduling is disabled"
        ...
        if state.consecutive_failures >= maximum_failures:
            next_due = parse_timestamp(state.next_due)
            ...
            if (
                ...
                return (
                    "provider is in extended failure backoff"
                )

        next_due = parse_timestamp(state.next_due)

        if next_due is not None and current < next_due:
            return "provider is not due yet"
    ...
    def _next_due_timestamp(
        ...
        *,
        ...
        if successful is False:
            ...
            if (
                successful is True
                and state.last_records == 0
            ):
                interval_minutes = max(
                    0,
                    _safe_int(
                        definition.get(
                            "empty_success_interval_minutes"
                        ),
    ...
    def _provider_state(
        ...
        normalized = normalize_key(provider)
        ...
        state = self.state.providers.get(normalized)

        if state is None:
            state = ProviderScheduleState(
                provider=normalized
            )
            self.state.providers[normalized] = state
    ...
    def _cursor_after_selection(
        ...
        try:
            absolute_index = eligible_names.index(
                last_selected
            )
        except ValueError:
...
def select_providers(
    *,
    ...
    requested: Iterable[str] | None = None,
    all_providers: bool = False,
    provider_budget: int = DEFAULT_PROVIDER_BUDGET,
) -> SchedulerSelection:
    """
