...
"""
Speciedex.org
static/tools/providers/tropicos.py

Tropicos provider plug-in.

This provider consumes a normalized or semi-normalized JSONL export configured
through providers.json. It is intended for Missouri Botanical Garden Tropicos
taxonomy and nomenclature, accepted names, synonyms, basionyms, authorship,
publications, type specimens, geographic distribution, common names,
...
the complete Tropicos source object is preserved under ``Taxon.extra["raw"]``.

Required provider configuration:

    {
        "name": "tropicos",
        "path": "static/data/providers/tropicos/taxa.jsonl"
    }
...
Optional configuration:

    {
        ...
        "source_url": "https://www.tropicos.org/"
    }
...
class Provider(BaseProvider):
    """File-backed Tropicos provider."""

    PROVIDER_NAME = "tropicos"

    DEFAULT_SOURCE_NAME = "Tropicos"
    DEFAULT_SOURCE_URL = "https://www.tropicos.org/"

    def fetch(self) -> Batch:
        """Read and normalize one resumable Tropicos JSONL batch."""

        source_path = self._source_path()

        if not source_path.exists():
            raise ProviderError(
                f"Tropicos export not found: {source_path}"
            )

        if not source_path.is_file():
            raise ProviderError(
                f"Tropicos path is not a file: {source_path}"
            )
    ...
    def _source_path(self) -> Path:
        """Resolve the configured Tropicos JSONL source path."""
        ...
        if not configured:
            raise ProviderError(
                "Tropicos provider requires a path."
            )
    ...
    def _normalize_record(
        ...
        *,
        ...
        """Normalize one Tropicos taxon or name record."""
        ...
        if not source_url:
            ...
            source_url = f"{base}/name/{provider_id}"
    ...
    def _extract_lineage(
        ...
        *,
        ...
        """Extract plant or fungal classification."""
    ...
    def _decode_cursor(cursor: str | None) -> int:
        ...
        try:
            ...
            raise ProviderError(
                f"Invalid Tropicos cursor: {cursor!r}."
            ) from error

        if offset < 0:
            raise ProviderError(
                "Tropicos cursor must be non-negative."
            )
