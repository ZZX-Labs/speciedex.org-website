...
"""
Speciedex.org
static/tools/providers/mycobank.py

MycoBank provider plug-in.
...
complete MycoBank object is preserved under ``Taxon.extra["raw"]``.

Required provider configuration:

    {
        "name": "mycobank",
        "path": "static/data/providers/mycobank/names.jsonl"
    }
...
Optional configuration:
    ...
    {
        "page_size": 1000,
        "source_name": "MycoBank",
        "source_url": "https://www.mycobank.org/"
    }
...
class Provider(BaseProvider):
    """File-backed MycoBank provider."""

    PROVIDER_NAME = "mycobank"

    DEFAULT_SOURCE_NAME = "MycoBank"
    DEFAULT_SOURCE_URL = "https://www.mycobank.org/"

    def fetch(self) -> Batch:
        """Read and normalize one resumable MycoBank JSONL batch."""

        source_path = self._source_path()

        if not source_path.exists():
            raise ProviderError(
                f"MycoBank export not found: {source_path}"
            )

        if not source_path.is_file():
            raise ProviderError(
                f"MycoBank path is not a file: {source_path}"
            )
    ...
    def _source_path(self) -> Path:
        """Resolve the configured MycoBank JSONL source path."""
        ...
        if not configured:
            raise ProviderError(
                "MycoBank provider requires a path."
            )
    ...
    def _normalize_record(
        ...
        *,
        ...
        """Normalize one MycoBank name or taxon record."""
    ...
    def _extract_synonyms(
        ...
        *,
        ...
        """Extract and deduplicate synonym-like fungal names."""
    ...
    def _normalize_authors(
        ...
        """Normalize fungal author records."""
    ...
    def _normalize_identifiers(
        ...
        """Normalize Index Fungorum, LSID, and other identifiers."""
    ...
    def _normalize_references(
        ...
        """Normalize fungal nomenclatural references."""
    ...
    def _normalize_rank(
        ...
        """Normalize fungal taxonomic ranks."""
    ...
    def _normalize_status(
        ...
        """Normalize MycoBank nomenclatural and taxonomic statuses."""
    ...
    def _infer_rank(
        ...
        """Infer fungal rank from name structure."""
    ...
    def _decode_cursor(
        ...
        try:
            ...
            raise ProviderError(
                f"Invalid MycoBank cursor: {cursor!r}."
            ) from error

        if offset < 0:
            raise ProviderError(
                "MycoBank cursor must be non-negative."
            )
