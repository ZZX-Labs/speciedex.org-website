...
"""
Speciedex.org
static/tools/providers/bryozoa_net.py

Bryozoa.net provider plug-in.

This provider consumes a normalized or semi-normalized JSONL export configured
through providers.json. It is intended for extant and fossil bryozoan taxonomy,
nomenclature, colony morphology, distribution, ecology, stratigraphy,
bibliography, media, and provenance data.

Each source record is normalized into the shared Speciedex Taxon contract while
the complete Bryozoa.net source object is preserved under
``Taxon.extra["raw"]``.
...
Required provider configuration:
    ...
    {
        "name": "bryozoa_net",
        "path": "static/data/providers/bryozoa-net/taxa.jsonl"
    }
...
Optional configuration:
    ...
    {
        "page_size": 1000,
        "source_name": "Bryozoa.net",
        "source_url": "https://www.bryozoa.net/"
    }
...
class Provider(BaseProvider):
    """File-backed Bryozoa.net provider."""

    PROVIDER_NAME = "bryozoa_net"

    DEFAULT_SOURCE_NAME = "Bryozoa.net"
    DEFAULT_SOURCE_URL = "https://www.bryozoa.net/"

    def fetch(self) -> Batch:
        """Read and normalize one resumable Bryozoa.net JSONL batch."""

        source_path = self._source_path()

        if not source_path.exists():
            raise ProviderError(
                f"Bryozoa.net export not found: {source_path}"
            )

        if not source_path.is_file():
            raise ProviderError(
                f"Bryozoa.net path is not a file: {source_path}"
            )
    ...
    def _source_path(self) -> Path:
        """Resolve the configured Bryozoa.net JSONL source path."""
        ...
        if not configured:
            raise ProviderError(
                "Bryozoa.net provider requires a path."
            )
    ...
    def _normalize_record(
        ...
        *,
        ...
        """Normalize one bryozoan taxon or nomenclatural record."""
        ...
        return Taxon(
            ...
            phylum=lineage.get("phylum", "Bryozoa"),
            ...
            extra={
                ...
                "programme": "bryozoa_net",
                "reference_only": True,
                "bryozoa_net_id": provider_id,
                ...
                "taxonomy": {
                    "subclass": normalize_space(
                        self._first_value(raw, "subclass")
                    ),
                    "superorder": normalize_space(
                        self._first_value(raw, "superorder")
                    ),
                ...
                "colony": {
                    ...
                    "substrate_attachment": normalize_space(
                        self._first_value(
                            raw,
                            "substrate_attachment",
                            "substrateAttachment",
                        )
                    ),
                    "mineralogy": self._list_value(
                        self._first_value(
                            raw,
                            "mineralogy",
                            "skeleton_mineralogy",
                    ...
                    "skeletal_composition": normalize_space(
                        self._first_value(
                            raw,
                            "skeletal_composition",
                            "skeletalComposition",
                        )
                    ),
                    "zooid_arrangement": normalize_space(
                        self._first_value(
                            raw,
                            "zooid_arrangement",
                            "zooidArrangement",
                    ...
                    "avicularia": normalize_space(
                        self._first_value(raw, "avicularia")
                    ),
                    "ovicells": normalize_space(
                        self._first_value(raw, "ovicells")
                    ),
                    "spines": normalize_space(
                        self._first_value(raw, "spines")
                    ),
                },
                "morphology": {
                    ...
                    "zooid_length_mm": self._normalize_range(
                        self._first_value(
                            raw,
                            "zooid_length_mm",
                            "zooidLengthMm",
                    ...
                    "zooid_width_mm": self._normalize_range(
                        self._first_value(
                            raw,
                            "zooid_width_mm",
                            "zooidWidthMm",
                ...
                "fossil_record": {
                    ...
                    "formation": normalize_space(
                        self._first_value(raw, "formation")
                    ),
                    "member": normalize_space(
                        self._first_value(raw, "member")
                    ),
                    "localities": self._normalize_regions(
                        self._first_value(
                            raw,
                            "fossil_localities",
                            "fossilLocalities",
    ...
    def _extract_lineage(
        ...
        *,
        ...
        """Extract bryozoan classification."""
    ...
    def _normalize_synonym_records(
        ...
        """Normalize synonym records with nomenclatural context."""
    ...
    def _normalize_range(
        ...
        """Normalize numeric minimum/maximum/optimum values."""
    ...
    def _normalize_regions(
        ...
        """Normalize geographic, marine, and fossil regions."""
    ...
    def _normalize_media(
        ...
        """Normalize specimen, SEM, light-microscopy, plate, and map media."""
    ...
    def _normalize_identifiers(
        ...
        *,
        ...
        """Normalize Bryozoa.net and external identifiers."""
        ...
        known_fields = {
            ...
            "itis_tsn": "ITIS",
            "itisTsn": "ITIS",
            "paleobiodb_id": "Paleobiology Database",
            "paleobiodbId": "Paleobiology Database",
    ...
    def _normalize_references(
        ...
        """Normalize bryozoan literature and nomenclatural references."""
    ...
    def _normalize_rank(value: Any) -> str:
        """Normalize bryozoan taxonomic ranks."""
    ...
    def _normalize_status(value: Any) -> str:
        """Normalize bryozoan taxonomic and nomenclatural statuses."""
    ...
    def _decode_cursor(cursor: str | None) -> int:
        ...
        try:
            ...
            raise ProviderError(
                f"Invalid Bryozoa.net cursor: {cursor!r}."
            ) from error

        if offset < 0:
            raise ProviderError(
                "Bryozoa.net cursor must be non-negative."
            )
