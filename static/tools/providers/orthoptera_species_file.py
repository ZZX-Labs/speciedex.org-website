...
"""
Speciedex.org
static/tools/providers/orthoptera_species_file.py

Orthoptera Species File (OSF) provider plug-in.

This provider consumes a normalized or semi-normalized JSONL export configured
through providers.json. It is intended for Orthoptera Species File taxonomy,
accepted names, synonymy, full orthopteran classification, nomenclature,
type-specimen metadata, geographic distribution, common names, references,
...
the complete Orthoptera Species File source object is preserved under
``Taxon.extra["raw"]``.

Required provider configuration:

    {
        "name": "orthoptera_species_file",
        "path": "static/data/providers/orthoptera-species-file/taxa.jsonl"
    }
...
Optional configuration:
    ...
    {
        "page_size": 1000,
        "source_name": "Orthoptera Species File",
        "source_url": "https://orthoptera.speciesfile.org/"
    }
...
class Provider(BaseProvider):
    """File-backed Orthoptera Species File provider."""

    PROVIDER_NAME = "orthoptera_species_file"

    DEFAULT_SOURCE_NAME = "Orthoptera Species File"
    DEFAULT_SOURCE_URL = "https://orthoptera.speciesfile.org/"

    def fetch(self) -> Batch:
        """Read and normalize one resumable OSF JSONL batch."""

        source_path = self._source_path()

        if not source_path.exists():
            raise ProviderError(
                f"Orthoptera Species File export not found: {source_path}"
            )

        if not source_path.is_file():
            raise ProviderError(
                f"Orthoptera Species File path is not a file: {source_path}"
            )
            ...
            for physical_line, line in enumerate(handle, start=1):
                ...
                try:
                    ...
                    if bool(self.definition.get("strict", False)):
                        raise ProviderError(
                            f"Invalid Orthoptera Species File JSON at "
    ...
    def _source_path(self) -> Path:
        ...
        if not configured:
            raise ProviderError(
                "Orthoptera Species File provider requires a path."
            )
    ...
    def _normalize_record(
        ...
        *,
        ...
        """Normalize one OSF taxon or nomenclatural record."""
        ...
        if not source_url:
            ...
            source_url = f"{base}/Taxa/{provider_id}/Overview"
        ...
        return Taxon(
            ...
            order=lineage.get("order", "Orthoptera"),
            ...
            extra={
                ...
                "programme": "orthoptera_species_file",
                "reference_only": True,
                "osf_id": provider_id,
    ...
    def _extract_lineage(
        ...
        *,
        ...
        lineage = {
            ...
            "class": normalize_space(raw.get("class")) or "Insecta",
            "order": normalize_space(raw.get("order")) or "Orthoptera",
            "suborder": normalize_space(raw.get("suborder")),
            "infraorder": normalize_space(raw.get("infraorder")),
            "superfamily": normalize_space(raw.get("superfamily")),
    ...
    def _normalize_identifiers(
        ...
        *,
        ...
        known_fields = {
            "osf_id": "Orthoptera Species File",
            "osfId": "Orthoptera Species File",
            "lsid": "LSID",
    ...
    def _decode_cursor(cursor: str | None) -> int:
        ...
        try:
            ...
            raise ProviderError(
                f"Invalid Orthoptera Species File cursor: {cursor!r}."
            ) from error

        if offset < 0:
            raise ProviderError(
                "Orthoptera Species File cursor must be non-negative."
            )
