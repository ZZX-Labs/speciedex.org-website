...
"""
Speciedex.org
static/tools/providers/world_echinoderm_database.py

World Echinoderm Database provider plug-in.

This provider consumes a normalized or semi-normalized JSONL export configured
through providers.json. It is intended for World Echinoderm Database / WoRMS
taxonomy, Aphia identifiers, accepted names, synonymy, full echinoderm
classification, nomenclature, marine/freshwater/terrestrial environment flags,
distributions, traits, references, media, external identifiers, and provenance
metadata.
...
Required provider configuration:
    ...
    {
        "name": "world_echinoderm_database",
        "path": "static/data/providers/world-echinoderm-database/taxa.jsonl"
    }
...
Optional configuration:
    ...
    {
        "page_size": 1000,
        "source_name": "World Echinoderm Database",
        "source_url": "https://www.marinespecies.org/echinoidea/"
    }
...
class Provider(BaseProvider):
    """File-backed World Echinoderm Database provider."""

    PROVIDER_NAME = "world_echinoderm_database"

    DEFAULT_SOURCE_NAME = "World Echinoderm Database"
    DEFAULT_SOURCE_URL = "https://www.marinespecies.org/echinoidea/"

    def fetch(self) -> Batch:
        """Read and normalize one resumable echinoderm JSONL batch."""

        source_path = self._source_path()

        if not source_path.exists():
            raise ProviderError(
                f"World Echinoderm Database export not found: {source_path}"
            )

        if not source_path.is_file():
            raise ProviderError(
                f"World Echinoderm Database path is not a file: {source_path}"
            )
            ...
            for physical_line, line in enumerate(handle, start=1):
                ...
                try:
                    ...
                    if bool(self.definition.get("strict", False)):
                        raise ProviderError(
                            f"Invalid World Echinoderm Database JSON at "
    ...
    def _source_path(self) -> Path:
        ...
        if not configured:
            raise ProviderError(
                "World Echinoderm Database provider requires a path."
            )
    ...
    def _normalize_record(
        ...
        *,
        ...
        """Normalize one echinoderm taxon record."""
        ...
        return Taxon(
            ...
            phylum=lineage.get("phylum", "Echinodermata"),
            ...
            extra={
                ...
                "programme": "world_echinoderm_database",
                "reference_only": True,
                ...
                "ecology": {
                    ...
                    "regenerative_capacity": normalize_space(
                        self._first_value(
                            raw,
    ...
    def _extract_lineage(
        ...
        """Extract full echinoderm classification."""

        lineage = {
            "kingdom": normalize_space(raw.get("kingdom")) or "Animalia",
            "phylum": normalize_space(raw.get("phylum")) or "Echinodermata",
            "subphylum": normalize_space(raw.get("subphylum")),
    ...
    def _normalize_identifiers(
        ...
        *,
        ...
        known_fields = {
            "aphia_id": "World Echinoderm Database",
            "AphiaID": "World Echinoderm Database",
            "aphiaId": "World Echinoderm Database",
            "worms_id": "WoRMS",
            "wormsId": "WoRMS",
            "gbif_id": "GBIF",
    ...
    def _decode_cursor(cursor: str | None) -> int:
        ...
        try:
            ...
            raise ProviderError(
                f"Invalid World Echinoderm Database cursor: {cursor!r}."
            ) from error

        if offset < 0:
            raise ProviderError(
                "World Echinoderm Database cursor must be non-negative."
            )
